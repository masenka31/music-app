...
import pandas as pd
from mysite.settings import knn_settings
...
def recommend_knn(input_ids, input_artists,prev_banned=[]):
    ...
    songs = pd.read_csv(knn_settings['knn_songs_info'], usecols=knn_settings['knn_used_columns'])
    reco = KNNRecommender()
    #reco.load_model(filename="data/KNN_trained_m.sav")
    reco.load_model(filename=knn_settings['knn_model_path'])
